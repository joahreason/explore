...
## it at the tent's tile centre, and frees it on waking. Runs in real time
...
func _draw() -> void:
	if _texture == null:
		return
	# Scale about the base of the tent (half a tile below its centre).
	var base := Vector2(0, _size * 0.5)
	var sy := stretch()
	draw_set_transform(base, 0.0, Vector2(2.0 - sy, sy))
