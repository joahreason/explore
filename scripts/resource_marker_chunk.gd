...
## and is never modified here. Positions are in tile units, relative to the
## world origin; this node sits at the chunk's pixel origin.

const FILL := Color(0.10, 0.32, 0.10)
...
## top-left tile, so positions can be drawn chunk-local.
func setup(instances: Array, origin_tile: Vector2i, tile_size: int, footprint_tiles: float) -> void:
	_positions.clear()
...
func _draw() -> void:
	for p in _positions:
		draw_circle(p, _radius + 1.0, OUTLINE)
		draw_circle(p, _radius, FILL)
