...
## streamed world. See scripts/world_gen.gd's water_body block for how the
## result is used.
##
## Determinism: for a body that fully fits within the budget (enclosed), the
## traversal always discovers the exact same tile set regardless of which
## tile triggered it, so area/perimeter/compactness/connected_to_ocean are
## stable no matter the query order - verified in the test script. For a
## body that exceeds the budget (open/unbounded), only the boolean "exceeded
## the cap" is trusted; WorldGen falls back to its existing low-frequency
## noise field for anything position-sensitive about *which* large body this
## is (ocean vs. sea flavor), since a capped BFS's shape is not
...
var _coarse_cache: Dictionary = {}  # Vector2i (coord/64) -> Result, for capped/open bodies
const COARSE_CELL := 64
...
func classify(
	...
	var coarse := Vector2i(floori(float(wx) / COARSE_CELL), floori(float(wy) / COARSE_CELL))
	if _coarse_cache.has(coarse):
		return _coarse_cache[coarse]
			...
			if e < sea_level:
				...
				if queue.size() > fill_budget:
					return {
						...
						"compactness": 0.0, "connected_to_ocean": false, "members": [],
					}
			else:
				boundary.append(n)
