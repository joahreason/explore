...
## the sun crosses from east to west between GameClock.SUNRISE_HOUR and
## SUNSET_HOUR, so shadows start long and pointing west (left), shorten
## towards noon and end long and pointing east; at night the moon does the
## same, much fainter. Both
...
static func at(hour: float) -> Vector3:
	...
	var sunrise := GameClock.SUNRISE_HOUR
	var sunset := GameClock.SUNSET_HOUR
