...
- **What World places** - trees as sprites (oak, pine, birch and mangrove with new art; olive, willow, palm from the one-bit tileset; tinted by each species' `sprite_color`), ore outcrops (iron, copper, coal) and rocks (granite, sandstone, basalt - by bedrock) as tileset sprites too, as are berry bushes, reeds and cattails; as are clay and salt outcrops and, on coasts, shells, river-mouth mud, beach grass, salt marsh and mangroves. Hidden when zoomed far out.
...
(Base Biome - a region-outline overlay - is available via the `B` key but left out of the dropdown for now in favor of click-to-inspect. The per-resource debug views - Oak Suitability/Density/Placement, Tree Cover, Tree/Rock/Berry Placement - still exist but are left out of the dropdown; give one a `label` in the `VIEWS` table of `scripts/world/view_modes.gd` to list it when tuning a resource.)
...
- **`chunk_manager.gd`** and its modules in `scripts/world/` (see `docs/architecture.md`) - infinite chunk streaming (16x16 tiles/chunk, 16 px tiles), built on a worker thread on desktop and in small per-frame steps on web. Builds one `Sprite2D` per loaded chunk from whichever color function the current view mode selects, plus an optional text-label overlay for label views. Also handles LOD: at low zoom each chunk samples on a coarser grid (1 sample per 2x2/4x4/8x8 tile block) and the sprite scales back up, since zooming out needs more chunks, not fewer.
...
- **`tools/generate_sfx.gd`** - writes the synthesized sound effects to `assets/sfx/` (deterministic; tweak a recipe and re-run `$GODOT --headless --path . --script res://tools/generate_sfx.gd`). So far `footstep.wav`: one short (60 ms), light brush of smoothed, band-passed noise (mostly 400 Hz - 2 kHz) with a rounded envelope - no boom, no tone, no click. (`harvest.wav` was removed on request, 2026-09-25.)
- **`player.gd`** - the player (each landing step plays the footstep at -26 dB with its pitch varied 0.82-1.18, never within 0.06 of the last, and +-1.5 dB): walks freely, not tile by tile, along a list of world points (`walk()`, `walk_speed` tiles per real second) - its position (the node's pivot) is its feet, the sprite drawn above; a small hop (`HOP_PX`; the cast shadow stays on the ground) and a footstep every `STEP_TILES` walked; `tile()` is the tile its feet are on. `ChunkManager._on_map_tapped()` plans the route with `navigation.gd` (`Navigation.find_path()`: A* over walkable tiles, 8 directions, no corner cutting, `MAX_PATH_NODES`, else the closest tile found; walkability is recorded as chunk images bake at LOD 1, else sampled), smooths it into straight legs (`Navigation.smooth_path()`) ending at the tapped point or beside a tapped resource, and harvests a tapped resource on arrival; `teleport_player()` (biome travel, tests) lands on the nearest dry tile. The exact position is saved per seed with the changes and time.
...
Art: the ground is one colour per tile (`terrain_surface.gd`), drawn by `shaders/terrain.gdshader` (water motion, surf, ground borders, seasonal grass). Placed resources and landmark parts are sprites: most are cut from the one-bit sheet `urizen_onebit_tileset__v2d0.png` and tinted by `scripts/render/resource_marker_chunk.gd` (`tileset.tres` describes the same sheet layout, but no scene uses it as a TileSet); the player, oak, birch, mangrove and pine (spruce) have new 16 px art in `assets/sprites/`.
...
Pushing to `main` triggers `.github/workflows/web-build.yml`, which runs the fast test suites (also on every pull request), then exports the Web preset with Godot 4.7.2 and deploys it to GitHub Pages automatically. There's no manual export step.
